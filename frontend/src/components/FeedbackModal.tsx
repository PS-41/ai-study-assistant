...
import ReviewsListModal from "./ReviewsListModal"; // Import the new component
...
export default function FeedbackModal({ onClose }: Props) {
  ...
  // State for the nested reviews modal
  ...
  return (
    <>
          ...
          <div className="text-center mb-6">
            ...
            {stats && (
              <button 
                ...
                className="group mt-1 px-3 py-1 rounded-full hover:bg-gray-50 transition-colors text-sm text-gray-500 flex items-center justify-center gap-1.5 mx-auto"
                title="View reviews"
              >
                <span>Current Rating:</span>
                <span className="font-semibold text-yellow-500">{stats.average}</span> 
                <span className="text-yellow-400 text-xs">★</span> 
                <span className="group-hover:text-blue-600 group-hover:underline transition-colors decoration-blue-600/30 underline-offset-2">
                  ({stats.count} reviews)
                </span>
              </button>
            )}
      ...
      {/* Render the Reviews List Modal if active */}
      {showReviewsList && <ReviewsListModal onClose={() => setShowReviewsList(false)} />}
    </>
  );
}