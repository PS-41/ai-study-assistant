...
export default function CoursesPage() {
  ...
  // Require selection helper (for right panel buttons)
  ...
  return (
    <div className="bg-gray-50">
      <div className="max-w-screen-2xl mx-auto w-full flex gap-6 px-6 py-6 h-[calc(100vh-140px)]">
        {/* Sidebar */}
        <aside className="w-64 bg-white border rounded-xl flex flex-col flex-shrink-0 overflow-hidden shadow-sm">
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-1">
            {courses.map((c) => (
              <div
                key={c.id}
                className={`group relative w-full flex items-center px-3 py-2 rounded-md text-sm gap-2 transition-colors ${
              ...
              >
                {isSelectMode && (
                  <button
                    ...
                    className={`flex-shrink-0 text-gray-400 hover:text-blue-600 transition ${
                      isCourseFullySelected(c.id) ? "text-blue-600" : ""
                    }`}
                  >
                ...
                <div className="hidden group-hover:flex gap-1 absolute right-2 bg-white/90 p-0.5 rounded border shadow-sm">
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      setRenamingItem({ type: "course", id: c.id, name: c.name });
                    }}
                  ...
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      setDeletingItem({ type: "course", id: c.id, name: c.name });
                    }}
        ...
        {/* Main */}
        <main className="flex-1 flex flex-col min-w-0 rounded-xl border bg-white shadow-sm overflow-hidden">
          {selectedCourse ? (
            <>
              <header className="bg-white border-b px-6 py-4 flex items-start justify-between flex-shrink-0">
                <div>
                  ...
                  <>
                    ...
                    {topics.map((t) => (
                      <TopicSection
                        ...
                        onToggle={toggleDocSelection}
                        onToggleBatch={toggleBatch}
                        expanded={!!expandedTopics[String(t.id)]}
                        onToggleExpand={() =>
                          setExpandedTopics((p) => ({
                            ...p,
                            [String(t.id)]: !p[String(t.id)],
                          }))
                        }
        ...
        <aside className="w-80 bg-white border rounded-xl shadow-md p-5 flex flex-col gap-6 flex-shrink-0">
          ...
          <div className="space-y-3 mb-2">
            ...
            <button
              onClick={() =>
                requireSelection(
                  "Select one or more documents in this course to generate a quiz.",
            ...
            <button
              onClick={() =>
                requireSelection(
                  "Select one or more documents in this course to generate flashcards.",
            ...
            >
              ...
              <div>
                ...
                <div className="text-xs text-gray-500">Memorize concepts</div>
              </div>
            </button>
            ...
            <button
              onClick={() =>
                requireSelection(
                  "Select one or more documents in this course to generate summaries.",
        ...
        {showTopicModal && selectedCourseId && (
          <CreateTopicModal
        ...
        <div className="flex items-center gap-3 flex-1">
          ...
          <div onClick={onToggleExpand} className="cursor-pointer flex items-center gap-3">
            <div>
              ...
              {subtitle && (
                <span className="ml-2 text-xs text-gray-500 font-normal">- {subtitle}</span>
              )}
          ...
          {docs.map((doc: Doc) => {
            ...
            return (
              <div
                ...
                className={`flex items-center p-3 gap-3 transition-colors cursor-pointer ${
                  isSelected ? "bg-blue-50" : "hover:bg-gray-50"
                }`}
              >
                <div
                  className={`flex-shrink-0 ${
                    isSelectMode ? "w-6" : "w-0 overflow-hidden"
                  } transition-all`}
                >
                  <button
                    onClick={(e) => {
...
function AddDocModal({ courseId, topicId, currentDocs, onClose, onSuccess }: any) {
  ...
  async function save() {
    ...
    try {
      ...
      setError("Something went wrong while adding documents. Please try again.");
    } finally {
      setSaving(false);
    }
  }
